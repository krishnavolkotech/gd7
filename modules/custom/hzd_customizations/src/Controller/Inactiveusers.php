 ...
 */
  function inactive_users() {
	...
	$query = \Drupal::database()->select('users', 'u');
        $query->addFields('u', array('access', 'name'));
        $query->condition('iu.uid', $uid , '=');
        $query->range(1, 0);
       ...
       // $u = db_fetch_object(db_query('SELECT access, name FROM {users} WHERE uid = %d', $uid));
		...
		$query = \Drupal::database()->delete('inactive_users');
                ...
                $query->execute();
                $message = 'recent user activity: ' . $u->name . 'removed from inactivity list. '  . Link::createFromRoute($this->t('edit user') , '/admin/people/list');
    ...
    return $this->t("Checked inactive users");
	...
	function  notify_admin_inactive_accounts() {
		...
		$query->addField('iuf', 'value');
               ...
               // !db_fetch_object(db_query('SELECT uid FROM {inactive_users} WHERE uid = %d AND notified_admin = 1', $user->uid));
		...
		Inactiveuserhelper::inactive_user_mail(
		 t( \Drupal::config('system.site')->get('site_name') . ' Inactive users'), 
                  Inactiveuserhelper::inactive_user_mail_text('notify_admin_text'), $notify_time, NULL, $user_list);
		unset($user_list);
	      }
	    }
	}
	...
	//	$inactive_flag = db_result(db_query("SELECT value from {inactive_user_flag} WHERE user_id = %d", $user->uid));
		...
		//  db_query('UPDATE {inactive_users} SET notified_user = 1 WHERE uid = %d', $user->uid);
		  ...
		  Inactiveuserhelper::inactive_user_mail($inactive_user_notify_mail_subject, $inactive_user_notify_text, $notify_time, $user, NULL);
                ...
                $edit_user = Link::createFromRoute($this->t('edit user') , "user/$user->uid/edit", array('query' => array('destination' => 'admin/user/user')))->toString();
                 echo '<pre>'; print_r($edit_user);  exit;
                 
                  $message = "user $user->name notified of inactivity " . $u->name;
...
//               $type, $message, $variables, $severity, $link)
	...
	function warn_to_block_inactive_accounts() {
         ...
         echo '<pre>';  print_r($block_time);  exit;
	...
	//	$inactive_flag = db_result(db_query("SELECT value from {inactive_user_flag} WHERE user_id = %d", $user->uid));
		...
		if ($inactive_flag != 1 && $user->uid && !$inactive_users_uid && ($user->created < (time() - $block_time))) {
		  ...
		  Inactiveuserhelper::inactive_user_mail($inactive_user_block_warn_mail_subject, $inactive_user_block_warn_text, $warn_time, $user, NULL);
                 ...
                 $message ="user $user->name warned will be blocked due to inactivity " . $u->name . Link::createFromRoute($this->t('edit user') , "user/$user->uid/edit", array('query' => array('destination' => 'admin/user/user')));
	...
	function block_inactive_accounts() {
		...
		//      watchdog('user', 'user %user blocked due to inactivity', array('%user' => $user->name), WATCHDOG_NOTICE, l(t('edit user'), "user/$user->uid/edit", array('query' => array('destination' => 'admin/user/user'))));
                 ...
                 $message ="user $user->name blocked due to inactivity" .  $user->name . Link::createFromRoute($this->t('edit user') , "user/$user->uid/edit", array('query' => array('destination' => 'admin/user/user')));
                 ...
                 \Drupal::logger('inactive_user')->notice($message);
                  ...
                  $warned_user_block_timestamp_uid = \Drupal::database()->select('inactive_users', 'iuf');
		...
		//    watchdog('user', 'user %user blocked due to inactivity', array('%user' => $user->name), WATCHDOG_NOTICE, l(t('edit user'), "user/$user->uid/edit", array('query' => array('destination' => 'admin/user/user'))));
	      ...
	      if (isset($user_list)) {
		Inactiveuserhelper::inactive_user_mail(
		 t( \Drupal::config('system.site')->get('site_name') . ' Blocked users'), 
	...
	//	$inactive_flag = db_result(db_query("SELECT value from {inactive_user_flag} WHERE user_id = %d", $user->uid));
		...
		$query->addField('iu', 'uid');
		  ...
		  if ($inactive_user_preserve_content && _inactive_user_with_content($user->uid)) {
		    $protected = 1;
		  }
		  else {
		    $protected = 0;
		...
		//    watchdog('user', 'user %user blocked due to inactivity', array('%user' => $user->name), WATCHDOG_NOTICE, l(t('edit user'), "user/$user->uid/edit", array('query' => array('destination' => 'admin/user/user'))));
                      ...
                      $message ="user $user->name warned will be deleted due to inactivity" .  $user->name . Link::createFromRoute($this->t('edit user') , "user/$user->uid/edit", array('query' => array('destination' => 'admin/user/user')));
	...
	function delete_inactive_accounts() {
	  $delete_time = \Drupal::config('inactive_user.settings')->get('inactive_user_auto_delete');	
		...
		if ($inactive_flag != 1 && $user->uid && $inactive_user_auto_delete_warn && $warned_user_delete_timestamp_uid || (!$inactive_user_auto_delete_warn) && ($user->created < (time() - $delete_time))) {
	      ...
	      if ($user_list) {
		Inactiveuserhelper::inactive_user_mail(
		 t( \Drupal::config('system.site')->get('site_name') . ' Deleted accounts'), 
                  Inactiveuserhelper::inactive_user_mail_text('block_notify_admin_text'), $delete_time, NULL, $user_list);
                unset($user_list);
	      }
	    }
	}
