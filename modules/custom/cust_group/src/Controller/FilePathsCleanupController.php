...
//      ->range(0, 15)
    ...
    $fileChunks = array_chunk($d6Files, 10);
...
//      ),
  ...
  public static function migrateFile($d6Files, $context) {
//    $d6File = reset($data);
    global $base_url;
...
//      pr($nodesQuery);exit;
      foreach ((array) $nodesQuery as $item) {
        ...
        $update = $d8Db->update('node__body')
          ...
          ->execute();
        \Drupal::logger('migr-upd')
          ->debug($item->entity_id . '==' . $d8FilePath, ['migr-upd']);
      }
...
//        pr($replacedData);exit;
        $d8Db->update('node_revision__body')
          ...
          ->execute();
        \Drupal::logger('migr-rev')
          ->debug($item->revision_id . '==' . $d8FilePath, ['migr-rev']);
      }
    }
