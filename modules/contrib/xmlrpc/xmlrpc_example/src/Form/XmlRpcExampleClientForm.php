...
class XmlRpcExampleClientForm extends FormBase {
  ...
  public function submitInformation(array &$form, FormStateInterface $form_state) {
    ...
    if ($result === FALSE) {
      ...
      $this->messenger()->addStatus($this->t('The XML-RPC server returned this response: <pre>@response</pre>', [
  ...
  public function submitAdd(array &$form, FormStateInterface $form_state) {
    ...
    if ($result === FALSE) {
      ...
      $this->messenger()->addStatus($this->t('The XML-RPC server returned this response: @response', [
        '@response' => print_r($result, TRUE),
      ]));
    }
  }
